<?php
  ...
  if($auth->isAdmin()) {
    ...
    $error = false;
    $is_dialog = false;
		$is_dialog_string = 'false';
    if(!empty($_GET['dialog']) && $_GET['dialog'] == 1) {
      $is_dialog = true;
			$is_dialog_string = 'true';
    }
...
if($error == false) {
  ...
  if($table_name != 'images') {
    ...
    ';
          ...
          if(InferCMS::endsWith($column_name, '_id')) {
            ...
            echo '<label>' . Inflector::humanize($column_name) . '</label><br>';
						if($column['DATA_TYPE'] == 'tinyint') {
							echo '<input class="input-large" value="1" type="checkbox" id="input_' . $column_name . '" name="' . $column_name . '">';
							...
							echo '<input type="text" id="input_' . $column_name . '" name="' . $column_name . '">';
							$script .= '
							...
							echo '<input class="input-large" type="text" id="input_' . $column_name . '" name="' . $column_name . '">';
						}
      ...
      ?>
      <button name="submit" onclick="submitNewForm('<?php echo $model; ?>', <?php echo $is_dialog_string; ?>)" id="dialog-submit-button_<?php echo $model; ?>" type="submit" class="btn btn-sm btn-info pull-right" data-loading-text="Loading...">Save</button>
    </div>
...
jQuery(function($) {
  ...
  <?php } else { // is form for images
		...
		include_once('includes/imageForm.php');
